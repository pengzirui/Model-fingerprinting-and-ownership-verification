...
def concanate_fingerprint(net, data_sets, pert, label, b_z = 32, nb_class = 10):
    ...
    features = torch.zeros( n_clusters, 2 * nb_class * nb_neighbors)
    ...
    data_loader = torch.utils.data.DataLoader(new_dataset, b_z, shuffle=True)
    return data_loader 
...
def regroup_datapoints(data_sets)
    ...
    n_clusters = len(data_sets)
    new_set_image = [torch.zeros((n_clusters,1,28,28)) for i in range(n_neighbors)]
    new_set_label = [torch.zeros((n_clusters)) for i in range(nb_neighbors)]
    new_datasets = []
